...
/**
 * Manages SenseNova API key via VS Code SecretStorage (secure) with
...
export class AuthManager {
	...
	async getApiKey(): Promise<string | undefined> {
		...
		const config = vscode.workspace.getConfiguration('sensenova-copilot');
	...
	async promptForApiKey(): Promise<boolean> {
		const apiKey = await vscode.window.showInputBox({
			prompt: 'Enter your SenseNova Token Plan API key',
			placeHolder: 'sk-...',
			password: true,
			ignoreFocusOut: true,
		...
		if (apiKey) {
			...
			vscode.window.showInformationMessage('SenseNova API key saved securely.');
			return true;
		}
